...
class MicroServicesRCA():
    ...
    def write_to_experiment_log(self, index, type, name, nodes, start, end, info):
    ...
    def stress_cpu_nodes(self, nodes, nstressors, timeout):
        self.write_to_experiment_log(current_milli_time(), "anomaly", "stress_cpu_nodes", list(nodes), int(time.time()),
    ...
    def stress_disk_nodes(self, nodes, nstressors, timeout):
        self.write_to_experiment_log(current_milli_time(), "anomaly", "stress_disk_nodes", list(nodes), int(time.time()),
                                     int(time.time()) + timeout, "Stressors: {0}".format(nstressors))
    ...
    def stress_network_nodes(self, nodes, nstressors, timeout):
        self.write_to_experiment_log(current_milli_time(), "anomaly", "stress_network_nodes", list(nodes), int(time.time()),
                                     int(time.time()) + timeout, "Stressors: {0}".format(nstressors))
    ...
    def stress_lockbus_nodes(self, nodes, nstressors, timeout):
        self.write_to_experiment_log(current_milli_time(), "anomaly", "stress_lockbus_nodes", list(nodes), int(time.time()),
                                     int(time.time()) + timeout, "Stressors: {0}".format(nstressors))
    ...
    def stress_cache_nodes(self, nodes, nstressors, timeout):
        self.write_to_experiment_log(current_milli_time(), "anomaly", "stress_cache_nodes", list(nodes), int(time.time()),
                                     int(time.time()) + timeout, "Stressors: {0}".format(nstressors))
    ...
    def limit_upload_bandwidth_nodes(self,nodes,delay='100ms',delay_jitter='1ms',bandwidth='100kbps',loss_percent='1%'):
        self.write_to_experiment_log(current_milli_time(), "anomaly", "limit_upload_bandwidth_nodes", nodes, int(time.time()),
                                     0, "delay:{0},delay_jitter:{1},bandwidth:{2},loss_percent:{3}".format(delay,delay_jitter,bandwidth,loss_percent))
        self.anomaly_injector.limit_upload_bandwidth(nodes,delay,delay_jitter,bandwidth,loss_percent)

    def limit_upload_bandwidth_nodes_random(self,nnodes,delay='100ms',delay_jitter='1ms',bandwidth='100kbps',loss_percent='1%'):
        list_nodes = list(self.nodes)
        shuffle(list_nodes)
        self.limit_upload_bandwidth_nodes(set(list_nodes[0:nnodes]),delay,delay_jitter,bandwidth,loss_percent)
